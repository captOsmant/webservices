...
class Imager{
	...
	public static function removeFolder($id){
		...
		if(!mysql_error()) Logger::addReport("$APP_NAME > Imager.php",Logger::STATUS_INFO,"Удалена папка под номером $id из хранилища изображений.");
		else Logger::addReport("$APP_NAME > Imager.php",Logger::STATUS_ERROR,"Не удалось удалить папку с номером $id из хранилища изображений! ".mysql_error());
	...
	public static function removeImage($id){
		...
		$catalog->remove($id,1);
		if(!mysql_error()) Logger::addReport("$APP_NAME > Imager.php",Logger::STATUS_INFO,"Удалено изображение с номером $id из хранилища.");
		else Logger::addReport("$APP_NAME > Imager.php",Logger::STATUS_ERROR,"Не удалось удалить изображение с номером $id из хранилища! ".mysql_error());
