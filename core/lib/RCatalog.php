...
class RCatalog{
	...
	public function addItem($data, $parentId="", $nest=-1){
		...
		if(mysql_error()){
			...
			if(class_exists("Logger"))
				Logger::addReport("$APP_NAME > RCatalog::addItem(".print_r($data,true).", $parentId, $nest)", Logger::STATUS_ERROR,"Не удалось создать элемент в каталоге ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
			else throw new Exception("$APP_NAME > RCatalog::addItem(".print_r($data,true).", $parentId, $nest) - Ошибка! Не удалось создать элемент в каталоге ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
	...
	public function getItemsAt($nest=0){
		...
		if(mysql_error()){
			if(class_exists("Logger"))
				Logger::addReport("$APP_NAME > RCatalog::getItemsAt($nest)", Logger::STATUS_ERROR,"Не удалось получить элементы каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
			else throw new Exception("$APP_NAME > RCatalog::getItemsAt($nest). Не удалось получить элементы каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
			return [];		
		}
		
		$result = array();		
		while($raw = mysql_fetch_assoc($res)){ array_push($result,$raw);}
		return $result;
	}
	...
	public function getItemAt($id, $nest=0){
		...
		if(mysql_error()){
			if(class_exists("Logger"))
				Logger::addReport("$APP_NAME > RCatalog::getItemAt($id, $nest)", Logger::STATUS_ERROR,"Не удалось получить элемент каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
			else throw new Exception("$APP_NAME > RCatalog::getItemAt($id, $nest). Не удалось получить элемент каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
			return null;		
		}
	...
	public function getChildrenOf($id, $nest=0){
		...
		if(mysql_error()){
			if(class_exists("Logger"))
				Logger::addReport("$APP_NAME > RCatalog::getChildrenOf($id, $nest)", Logger::STATUS_ERROR,"Не удалось получить элементы каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
			else throw new Exception("$APP_NAME > RCatalog::getChildrenOf($id, $nest). Не удалось получить элементы каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
			return [];		
		}
	...
	public function getParentOf($id, $nest){
		...
		if(mysql_error()){
			if(class_exists("Logger"))
				Logger::addReport("$APP_NAME > RCatalog::getParentOf($id, $nest)", Logger::STATUS_ERROR,"Не удалось получить родителя элемента каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
			else throw new Exception("$APP_NAME > RCatalog::getParentOf($id, $nest). Не удалось получить родителя элемента каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
	...
	public function editValuesOf($id, $nest, $data){
		...
		foreach($data as $key => $value){
			...
			if (mysql_error()){
				if(class_exists("Logger"))
					Logger::addReport("$APP_NAME > RCatalog::editValuesOf($id, $nest,".print_r($data,true).")", Logger::STATUS_ERROR,"Не удалось изменить поле $key элемента каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
				else throw new Exception("$APP_NAME > RCatalog::editValuesOf($id, $nest,".print_r($data,true)."). Не удалось изменить поле $key элемента каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
				return false;
			}
	...
	public function addItemTo($id=0, $nest=-1){
			...
			if (mysql_error()){
				if(class_exists("Logger"))
					Logger::addReport("$APP_NAME > RCatalog::addItemTo($id, $nest)", Logger::STATUS_ERROR,"Не удалось добавить элемент в каталог ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
				else throw new Exception("$APP_NAME > RCatalog::addItemTo($id, $nest). Не удалось добавить элемент в каталог ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
				return false;
			} 
	...
	public function setParentId($id, $nest, $parent){
		...
		if (mysql_error()) {
			if(class_exists("Logger"))
				Logger::addReport("$APP_NAME > RCatalog::setParentId($id, $nest, $parent)", Logger::STATUS_ERROR,"Не удалось изменить родителя элемента каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
			else throw new Exception("$APP_NAME > RCatalog::setParentId($id, $nest, $parent). Не удалось изменить родителя элемента каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
			return false;
		}		
		return true;		
	}
	...
	public function removeItemAt($id, $nest=0){
		...
		if (mysql_error()) {
			if(class_exists("Logger"))
				Logger::addReport("$APP_NAME > RCatalog::removeItemAt($id, $nest)", Logger::STATUS_ERROR,"Не удалось удалить элемент из каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
			else throw new Exception("$APP_NAME > RCatalog::removeItemAt($id, $nest). Не удалось удалить элемент из каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
			return false;
		}	
	...
	public function getByQuery($id, $nest, $query){
		...
		if (mysql_error()) {
			if(class_exists("Logger"))
				Logger::addReport("$APP_NAME > RCatalog::getByQuery($id, $nest, $query)", Logger::STATUS_ERROR,"Не удалось получить элементы из каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
			else throw new Exception("$APP_NAME > RCatalog::getByQuery($id, $nest, $query). Не удалось получить элементы из каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
			return []; 
		}
	...
	public function getAllByQuery($nest, $query){
		...
		if (mysql_error()) {
			if(class_exists("Logger"))
				Logger::addReport("$APP_NAME > RCatalog::getAllByQuery($nest, $query)", Logger::STATUS_ERROR,"Не удалось получить элементы из каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
			else throw new Exception("$APP_NAME > RCatalog::getAllByQuery($nest, $query). Не удалось получить элементы из каталога ".$this->name.". Ошибка на уровне SQL: ".mysql_error()." << ".$this->lastQuery);
			return [];
		}
